...
def get_subject_uri_list_in_triples(triple_list: list, english_filter=True) -> list:
    res = []
    for triple in triple_list:
        if triple.s_type == "uri" and triple.s.startswith("http"):
            if english_filter is True and '.wikipedia.org/' in triple.s and not triple.s.startswith('http://en'):
                continue
            res.append(triple.s)
    return res


def get_object_uri_list_in_triples(triple_list: list, english_filter=True) -> list:
    res = []
    for triple in triple_list:
        if triple.o_type == "uri" and triple.o.startswith("http"):
            if english_filter is True and '.wikipedia.org/' in triple.o and not triple.o.startswith('http://en'):
                continue
            res.append(triple.o)
    return res
...
class KBRetriever:
    ...
    # relation_matching_service: RelationMatchingService
