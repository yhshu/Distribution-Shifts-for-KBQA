...
from utils.s_expr_util import get_function_in_lisp


class GrailQAJsonLoader(JsonLoader):

    def __init__(self, file_path: str, sample_size=None):
        ...
        if sample_size is not None:
            self.data = random.sample(self.data, min(sample_size, self.len))
            self.len = len(self.data)
    ...
    def get_sparql_by_idx(self, idx):
        if 'sparql_query' in self.data[idx]:
            return self.data[idx]['sparql_query']
        if 'sparql' in self.data[idx]:
    ...
    def get_function_by_idx(self, idx):
        if 'function' in self.data[idx]:
            return self.data[idx]['function']
        return get_function_in_lisp(self.get_s_expression_by_idx(idx))
    ...
    def get_ans_name_by_idx(self, idx):
        answers = self.data[idx]['answer']
        res = []
        for answer in answers:
            if type(answer) == str:
    ...
    def get_num_relation_by_idx(self, idx):
        if 'num_edge' in self.data[idx]:
            return self.data[idx]['num_edge']
        return None
    ...
    def get_golden_entity_mid_set(self):
        res = set()
        for idx in range(0, self.len):
            entities = self.get_golden_entity_by_idx(idx, only_id=True, only_label=False)
            res.update(entities)
        return res
    ...
    def get_golden_schema_domain_by_idx(self, idx) -> set:
    ...
    def get_class_friendly_name_dict(self):
    ...
    def get_golden_classes(self):
        res = set()
        for idx in range(self.len):
            res.update(self.get_golden_class_by_idx(idx, only_label=True))
        return res
    ...
    def entity_partial_ratio(self):
        ...
        for idx in range(0, self.len):
            ...
            for entity in golden_entities:
                res += fuzz.partial_ratio(entity.lower(), question)
                total += 1
        if total != 0:
            return res / total
        return 0
    ...
    def class_partial_ratio(self):
        ...
        for idx in range(0, self.len):
            ...
            question = self.get_question_by_idx(idx).lower()
            if golden_classes is None:
                continue
            for c in golden_classes:
                res += fuzz.partial_ratio(c, question)
                total += 1
        if total != 0:
            return res / total
        return 0
    ...
    def relation_partial_ratio(self):
        ...
        for idx in range(0, self.len):
            golden_relations = self.get_golden_relation_by_idx(idx)
            question = self.get_question_by_idx(idx).lower()
            if golden_relations is None:
                continue
            for r in golden_relations:
                res += fuzz.partial_ratio(r, question)
                total += 1
        if total != 0:
            return res / total
        return 0
...
def print_questions(dataloader):
    for idx in range(0, dataloader.len):
        print(dataloader.get_question_by_idx(idx))


def print_schema_items(dataloader: GrailQAJsonLoader):
    for idx in range(0, dataloader.len):
        classes = dataloader.get_golden_class_by_idx(idx, True)
        relations = dataloader.get_golden_relation_by_idx(idx)
        for c in classes:
            print(c)
        for r in relations:
            print(r)
...
if __name__ == "__main__":
    ...
    test_data = GrailQAJsonLoader(grailqa_test_path)
